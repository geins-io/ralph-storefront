  ...
  computed: {
    ...
    breadcrumbSchema() {
      ...
      if (this.isSale) {
        ...
        path = this.$config.baseUrl + '/l' + this.salePath;

        jsonld.itemListElement.push({
          '@type': 'ListItem',
          position,
          name: this.$t('BREADCRUMBS_SALE'),
          item: path
        });
      }
      ...
      if (this.parents.length && this.categoryTree.length) {
        ...
        for (let i = this.parents.length - 1; i >= 0; i--) {
          ...
          path = this.parents[i].canonicalUrl;

          if (this.isSale) {
            path = this.addSaleToPath(path);
          }
