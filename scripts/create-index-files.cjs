...
const pathTargetIndexJs = path.resolve(__dirname, '..', 'components-lib.js');
...
function createIndexFiles() {
  ...
  console.log('components-lib.js created');
}
...
function generateFilesContent() {
  ...
  for (const pathComponentVue of pathsVueAll) {
    ...
    const pathWithoutFileEnding = pathComponentVue.replace(/\.vue/g, '');
    caComponentName = caComponentName.replace(/.*\/(Mix.+)\.js/, '$1');
    const importLine = `import ${caComponentName} from "@/components/${pathWithoutFileEnding}";`;
